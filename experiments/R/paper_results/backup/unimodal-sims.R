...
gen_asci_data_type1 <- function(tot_sims, sim_no, n, p, eta, sigma) {
  # print(glue::glue("Simulatio¬n {sim_no} of {tot_sims}: Progress {scales::percent(sim_no/tot_sims)}\\n"))
  # Rademacher(p) contamination
  xi <- 2 * rbinom(n = n, size = 1, prob = p) - 1

  # True response
  # mu <- seq(from = n * eta, to = n, length.out = n) / n
  # mu <- seq(from = -2 * n * eta, to = 2 * n * eta, length.out = n) / n
  mu <- c(rep(0, 200), seq(from = n * eta, to = 2 * n , length.out = 800) / n)
  # print(glue::glue_collapse("mu values: {mu}\\n"))
  print(mu)
