...
out_ascifit2 <- out_ascifit1_r %>%
  dplyr::mutate(mu_hat_inv = purrr::map(.x = .data[["ascifit_out"]], .f = ~.x$mu_hat_inv),
...
out_ascifit2_mse_plt <- out_ascifit2_mse %>%
  ...
  ggplot2::geom_errorbar(ggplot2::aes(ymin = mean_mse - 2*se_mse,
                                      ymax = mean_mse + 2*se_mse), width = .5,
                         position = pd,
                         size = 2) +
  ggplot2::geom_line(position=pd) +
  ggplot2::geom_point(position=pd, size=4, shape=21) + # 21 is filled circle
  ggplot2::labs(title = latex2exp::TeX("Sample mean-MSE for estimating $\\mu$ using ASCIFIT"),
...
# Save plot
cowplot::ggsave2(filename = here::here("experiments", "R",
                                       "out_ascifit1_50_reps.png"),
